...
**Mock 模式**（`test_flow.py`，无 Key 离线）4/4 通过：
- ✅ 创建脚本 → `write_file`
- ✅ 危险命令 → 审批通过并执行
- ✅ `ask_user` → 提问并恢复 loop
- ✅ 工具并行调用（同时下发 2 个工具）

**真实模型**（`test_real.py`，gpt-5.2-codex @ api.apilio.ai/v1）4/4 通过：
- ✅ 写文件：创建 `sum1to100.py`（1-100 求和）
- ✅ 危险命令审批：`rm -rf /tmp/...` 被拦截 → 批准 → 执行（退出码 0）
- ✅ `ask_user`：模型提问"你最喜欢的水果是什么？"→ 回答"我喜欢吃苹果" → **继续 loop 写入 `favorite_fruit.md`**
- ✅ 工具并行：`list_dir` 后同时发起 4 个 `read_file`

**专项验证**（对照题目要求）：
- ✅ 沙箱隔离：`../`、`C:\Windows\...`、`D:\...` 等越界路径全部拒绝
- ✅ 危险命令黑名单：10 类破坏性命令全部命中，7 类安全命令全部放行
- ✅ 并发写冲突：10 个并发写同一文件，写锁串行化，无内容交错
- ✅ Mock 分支：6 种关键词触发场景全部正确

## 未完成 / 可改进

- 流式输出（token 级打字机）未做，当前为整段消息推送
- 会话持久化未做（内存态，重启丢失），可扩展 SQLite 存储
- 危险命令识别为启发式黑名单，可结合 LLM 语义判断降低误报/漏报
